...
notice_of_ban = function.plugins.report.notice_of_ban() #修改你禁言后通知的内容
                          
        
@on_command('ban', aliases=('定时禁言', 'ban', '1',), only_to_me=False)
async def ban(session: CommandSession):
    global number_of_group,notice_of_free
    message_type = session.ctx['message_type']
    duration = str(session.get('duration', prompt='明天几点钟禁言呀？'))
    if not duration.isdigit():
        await session.pause('时间应该是数字,例如：7，8，9')
    notice_of_free += str(session.get('notice_of_free', prompt='禁言后你想通知什么内容？'))
